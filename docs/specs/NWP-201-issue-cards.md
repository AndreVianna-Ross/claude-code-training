...
- `src/components/ui/payments/StatusBadge.tsx:10` one badge keyed by a status union (`LABELS`/`DOTS`/`VARIANTS`); `src/app/payments/[id]/page.tsx` the detail-page idiom: back link, `notFound()`, `<h1>`, `Divider`, `<dl>`.
- `src/components/` has `Drawer` (the only `@radix-ui/react-dialog` wrapper), `Button`, `Input`, `Select`, `Badge`, `Table`. **No Dialog**, despite `.claude/rules/components.md:9` claiming one; no progress bar. `vitest.config.ts:13` runs a **node** environment over `src/**/*.test.ts`, so pure modules and the data layer test, and no `.tsx` ever loads.
...
| Allowlist everything from a client before it reaches the store; reject early. | ORG-7; `api-routes.md:8`, `:13` | A hand-rolled POST sets a £50m limit on a merchant that does not exist |
| One error shape everywhere: a status that means what it says, a body safe to show. | `api-routes.md:11` | Callers branch on shape instead of on status |
...
| No inline `style`; Tailwind only. | `components.md:10` | The spend bar cannot use a computed width |
...
| `src/lib/card-number.ts` | add | The only module that ever holds a full PAN, so it stays small: `generateCardNumber` (injectable `random` so the extremes are testable), `luhnCheckDigit`, `isLuhnValid`, `maskedNumber`, `cardReference`, `TEST_BIN`. |
...
| `src/lib/card-number.test.ts`, `src/lib/cards.test.ts`, `src/data/cards.test.ts` | add | The generator over 500 samples plus its extremes; a reference independent of the number; every transition; a row per validation rejection; idempotent issuing and ordering against the real store. |
...
| `src/data/store.ts`, `src/data/generate.ts` | change | A `cards` slice and an `issuedRequests` index — both on the store, so a reload keeps them — and three cards seeded through the real generator pinned to `GENERATED_AT`: one active, one past 80% (the amber case), one frozen. |
...
| `src/app/cards/issue-dialog.tsx` | add | `Drawer` form → one-time reveal, cleared on close. One `Field` renders a Select or an Input, since the label/error/aria plumbing is identical. A replay shows the mask, not a second PAN. |
...
| `src/components/ui/payments/StatusBadge.tsx` | change | Card statuses join the existing badge union. No status name collides, so a second badge component would have been duplication. |
...
8. **A nickname is required and capped at 60 characters, and a limit must be a whole number.** The ticket names neither. A card with no nickname is unidentifiable in a list that shows no PAN, and a fractional minor unit is a money-rule violation arriving as valid JSON, so both are rejected as 400s beside the rules the ticket does name. **The category lock is optional** — absent, `null` and `""` all store `null` — because the ticket makes the lock a stretch goal rather than a requirement.

## Plan

Types and store slice → `src/lib` rules with their tests → `src/data/cards.ts` with its tests → route handlers → list, detail, dialog, actions → nav → browser verification → `/ship-ready` → `/pr` → push.

## Verification

| Criterion | How it is proven |
| --- | --- |
| CORE-4 generated numbers; Luhn on the test BIN | Automated: 500 samples asserted 16 digits, `4242`-prefixed and Luhn-valid, plus both extremes of `random` by injection |
| CORE-6 server-side validation | Automated: a row per rejection through `parseIssueRequest` — missing merchant, zero, negative, over-cap and fractional limits, wrong currency, unknown category, blank and over-long nickname, and four bad fields reported at once |
| RULE-3 state machine | Automated: every legal edge, `cancelled` as terminal, same-status refused, unknown card reported — through `transitionCard` against the real store |
| CORE-5 reveal once | Automated: `issueCard`'s returned record is asserted not to contain the number anywhere, and `reference` is proven independent of it by two seeded tests |
| Idempotency and ordering | Automated: one card per `requestId` across retries, distinct keys give distinct cards, newest-first with the id tie-break |
| CORE-1/2/3 issue, list, detail; stretch states | **By hand in the browser:** issue → the row appears → detail → freeze and unfreeze, plus the empty and error states |
| Every check | `tsc --noEmit`, `next lint` and `vitest run` (87 tests) on every push, via CI |

**The gap, stated plainly:** nothing automated drives the UI or asserts an HTTP status code. `vitest.config.ts:13` is a node environment, so no `.tsx` loads, and the routes are covered only through the pure parser and the data layer they delegate to. The status codes and the drawer were checked by hand. A `.tsx` environment and a route-level test are the first things the next ticket should add.
